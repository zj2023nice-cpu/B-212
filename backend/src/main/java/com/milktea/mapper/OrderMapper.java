...
public interface OrderMapper extends BaseMapper<Order> {
    ...
    @Select("SELECT COUNT(*) FROM orders WHERE status NOT IN (0, 3) AND DATE(create_time) = CURDATE()")
    Long countTodayOrders();

    @Select("SELECT COALESCE(SUM(pay_amount), 0) FROM orders WHERE status NOT IN (0, 3) AND DATE(create_time) = CURDATE()")
    BigDecimal sumTodaySales();

    @Select("SELECT COUNT(*) FROM orders WHERE status IN (0, 1)")
